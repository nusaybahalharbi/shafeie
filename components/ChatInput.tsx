...
export default function ChatInput({ onSend, isLoading, onStop, lang }: { onSend: (t: string) => void; isLoading: boolean; onStop: () => void; lang: "ar" | "en" }) {
      ...
      <div className="mx-auto max-w-[780px]">
        <form onSubmit={e => { e.preventDefault(); if (input.trim() && !isLoading) { onSend(input); setInput(""); } }} className="flex items-center gap-3">
          <input value={input} onChange={e => setInput(e.target.value)}
            placeholder={lang === "ar" ? "اسأل عن آية، موضوع، أو اطلب إرشاداً..." : "Ask about a verse, topic, or seek guidance..."}
        ...
        <p className="mt-2.5 text-center text-[10px] text-nur-text-faint/50">
          {lang === "ar" ? "مصادر موثقة فقط · بدون فتاوى · يرفض عند عدم اليقين" : "Verified sources only · No fatwas · Refuses when uncertain"}
        </p>
      </div>
    </div>
  );
