...
export default function AyahCard({ fields }: { fields: AyahFields }) {
  ...
  return (
    <div className="mb-5 overflow-hidden rounded-2xl border border-d-border [.light_&]:border-l-border bg-d-card [.light_&]:bg-l-card shadow-xl shadow-black/10 [.light_&]:shadow-black/5">
      {/* Quran verse - premium typography */}
      <div className="bg-gradient-to-br from-d-gold/[0.06] to-transparent [.light_&]:from-l-gold/[0.04] border-b border-d-border [.light_&]:border-l-border px-7 pb-6 pt-7" dir="rtl">
        <p className="font-quran text-[24px] leading-[2.6] text-d-gold [.light_&]:text-l-gold verse-glow">{fields.arabic}</p>
      </div>

      {/* Translation */}
      {fields.translation && (
        <div className="border-b border-d-border [.light_&]:border-l-border px-7 py-4" dir="ltr">
          <p className="font-en text-[14px] italic leading-relaxed text-d-text-dim [.light_&]:text-l-text-dim">{fields.translation}</p>
        </div>
      )}

      {/* Metadata */}
      <div className="flex flex-wrap gap-2 border-b border-d-border [.light_&]:border-l-border px-7 py-3">
      ...
      {/* Tafsir */}
      {fields.tafsir && (
        <div className="border-b border-d-border [.light_&]:border-l-border px-7 py-5">
          <div className="mb-2 font-body text-[11px] font-semibold text-d-gold [.light_&]:text-l-gold tracking-wide">التفسير — ابن كثير</div>
          <p className="font-body text-sm leading-[1.9] text-d-text-dim [.light_&]:text-l-text-dim" dir="auto">{fields.tafsir}</p>
        </div>
      )}
